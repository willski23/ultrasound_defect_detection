#!/usr/bin/env python
"""
Script to resume training with GPU optimizations.
This script loads an existing checkpoint and dataset info
and continues training with improved GPU performance.
"""
import os
import sys
import argparse
...
'''
# Use float32 for compatibility with compute capability 6.1
from tensorflow.keras.mixed_precision import set_global_policy
policy = 'float32'
set_global_policy(policy)
print(f"Precision policy set to: {policy}")

def load_dataset_from_info(dataset_info_path, img_size=(224, 224), batch_size=32):
    """
    ...
    def load_and_preprocess(img_path, mask_path):
        ...
        img = tf.cast(img, tf.float16) / 255.0  # Store images as float16
        ...
        mask = tf.cast(mask, tf.float16) / 255.0  # Store masks as float16
...
def resume_training(model_path, train_ds, val_ds, epochs=20, checkpoint_dir=None):
    """
    Resume training from a checkpoint with GPU optimizations.
    ...
    model_name = f"ultrasound_optimized_{timestamp}"
    ...
    # Resume training
    print(f"Resuming training for {epochs} epochs with GPU optimizations...")
    history = model.fit(
        train_ds,
...
def main():
    parser = argparse.ArgumentParser(description="Resume training with GPU optimizations")
